...
export function PatientCombobox({ patients, onSelect, defaultPatient }: PatientComboboxProps) {
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      >
        {selected?.isNew && (
          <UserPlus className="size-3.5 shrink-0 text-[#2e37a4]" />
        )}
        <input
          ref={inputRef}
          value={query}
        ...
        <div className="absolute z-50 mt-1 w-full rounded-lg border border-border bg-popover shadow-md overflow-hidden">
          <ul className="max-h-52 overflow-y-auto py-1">
            ...
            {filtered.map(p => (
              ...
              >
                <Check
                  className={cn('size-3.5 shrink-0', selected?.label === p.name && !selected.isNew ? 'opacity-100' : 'opacity-0')}
                />
                {p.name}
              </li>
            ))}
            {showCreate && (
              <li
                onMouseDown={e => { e.preventDefault(); createPatient() }}
