...
export function SummaryCards({ stats }: { stats: Stats }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      color: 'text-blue-600',
      details: [
        { label: 'Medications used', value: stats.uniqueMedCount },
      ],
    },
    {
      label: 'Expenses',
      ...
      {cards.map(card => {
        ...
        return (
          <div
            key={card.label}
            className={cn(
              'bg-white border rounded-lg p-4 space-y-1',
          ...
          >
            <div className="flex items-center justify-between gap-1">
              ...
              {hasDetails && (
                <ChevronDown
                  className={cn('size-3.5 text-gray-400 shrink-0 transition-transform', isExpanded && 'rotate-180')}
                />
              )}
            </div>
